...
begin
  ...
  l := Length(AStr);
  for i := 1 to l do
    if {CharInSet(AStr[i], ['0'..'9'])} AStr[i] in ['0'..'9'] then //изменено GoldMedium - D2009/D2010
...
begin
  ...
  for i := 1 to Length(AName) do
    if {CharInSet(AName[i], [' '..'-', '/', ':'..'@', '['..'^', '''', '{'..#127])}
      AName[i] in [' '..'-', '/', ':'..'@', '['..'^', '''', '{'..#127] then begin //изменено GoldMedium - D2009/D2010
      Result := false;
      Break;
    end;
end;
...
begin
  Result := true;
  for i := 1 to Length(AName) do
    if NOT {CharInSet(AName[i], ['A'..'Z', 'a'..'z', '0'..'9', '_', '.'])}
      (AName[i] in ['A'..'Z', 'a'..'z', '0'..'9', '_', '.']) then begin //изменено GoldMedium - D2009/D2010
      Result := false;
      Break;
    end;
end;

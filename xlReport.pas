{—обcтвенные исправлени€:
  - в процедуре class procedure MergeReports(Reports: array of TxlExcelReport; SheetPrefixes: array of string)
    добавлена возможность объединени€ отчетов с последующем их открытием в режиме
    предварительного просмотра, а не редактировани€ как раньше
    дл€ этого сделаны следующие изменени€:

    изменен вызов процедуры class procedure MergeReports(Reports: array of TxlExcelReport; SheetPrefixes: array of string); override;
    на class procedure MergeReports(Reports: array of TxlExcelReport; SheetPrefixes: array of string; const APreview: boolean = False); override;

  - в объ€влении класса TxlVCLDataSet = class(TxlAbstractDataSet)
    добавлено услови€ дл€ объ€влени€ типа переменной FBookmark
...
{ Event handlers }
  ...
  TxlOnBeforeWorkbookSave = procedure (Report: TxlReport;
    var WorkbookName, WorkbookPath: string; var Save: boolean) of object;
...
{ TxlReport class }
  ...
  public
    ...
    {$IFNDEF XLR_BCB}
    class procedure MergeReports(Reports: array of TxlExcelReport; SheetPrefixes: array of string; const APreview: boolean = False); override; //изменено GoldMedium
    {$ENDIF}
    // used interfaces
    property IXLSApp;
    property IWorkbooks;
...
begin
{
  ...
  xSep := DecimalSeparator;
  DecimalSeparator := '.';
  xVer := StrToFloat(x.version);
  DecimalSeparator := xSep;
  if xVer > 14 then
  begin
    b := x.Workbooks.Add;
    inherited Report;
...
class procedure TxlReport.MergeReports(Reports: array of TxlExcelReport;
  SheetPrefixes: array of string; const APreview: boolean = False); //изменено GoldMedium
begin
  inherited;
end;
{$ENDIF}

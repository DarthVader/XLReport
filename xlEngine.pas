...
const
  ...
  xlrVBASelectAllSheetsModule =
    ...
    '  Dim i, t As Integer' + vbCR +
    '  Dim SheetCount As Long' + vbCR +
    '  Dim Wbk As Workbook' + vbCR +
    '  Dim Arr As Variant' + vbCR +
    '  Set Wbk = Workbooks(Wbks(1))' + vbCR +
    '  SheetCount = 0' + vbCR +
    '  For i = 1 To Wbk.Sheets.Count' + vbCR +
    '      If Wbk.Sheets(i).Visible = True Then' + vbCR +
    '         SheetCount = SheetCount + 1' + vbCR +
    '      End If' + vbCR +
    '  Next' + vbCR +
    '  ReDim Arr(1 To SheetCount)' + vbCR +
    '  t = 0' + vbCR +
    '  For i = 1 To Wbk.Sheets.Count' + vbCR +
    '      If Wbk.Sheets(i).Visible = True Then' + vbCR +
    '         t = t + 1' + vbCR +
    '         Arr(t) = Wbk.Sheets(i).Name' + vbCR +
    '      End If' + vbCR +
    '  Next' + vbCR +
    '  Wbk.Sheets(Arr).Select' + vbCR +
    'End Sub' + vbCR;
  xlrVBASelectOneSheets = 'xlrSelectOneSheets';
  xlrVBASelectOneSheetsModule =
    'Public Sub xlrSelectOneSheets(Wbks As Variant)' + vbCR +
    '  Dim Wbk As Workbook' + vbCR +
    '  Dim SheetName As String' + vbCR +
    '  Set Wbk = Workbooks(Wbks(1))' + vbCR +
    '  If Wbk.Sheets.Count > 0 _' + vbCR +
...
//      IRange.Rows.Item[IRange.Rows.Count, EmptyParam].Delete(TOLEENum(xlShiftUp));
  end;
  _Clear(FIMergedRows);
...
begin
  ...
  if IRange.Rows.Count > (RowCount + 1) then begin
    ...
    IRange.Rows.Item[1, EmptyParam].Delete(TOLEENum(xlShiftUp));
  end;
  // FRanges emulation
  ...
  FRanges[1] := 1;
  if FDeleteSpecialRow then
    FRanges[2] := IRange.Rows.Count
  else
    FRanges[2] := IRange.Rows.Count - 1;
  MakeRootRanges;
  // xldebug
        ...
        if Source = Self then begin
          ...
          if Source.RangeType = rtRangeDetail then begin
            ...
            while not Source.XLDataSet.Eof do begin
              ...
              Source.FRecordCount := Source.FRecordCount + 1;
              {$IFDEF XLR_VCL}
            ...
            (Source.GetRoot.FBuff as TxlBuffer).CheckLimits(false, GetRoot);
          end
          else begin
            Source.PutRoot;
            Inc(i, Source.RowCount + 1);
...
procedure TxlExcelReport.DoOnBeforeWorkbookSave(var WorkbookFileName,
  WorkbookFilePath: string; var Save: boolean);
begin
  // abstract stub
end;
    ...
    for i := 1 to iCount do
      if UpperCase(IWorkbooks.Item[i].Name) = UpperCase(ShortFileName) then
        if State = rsReport then
          ...
          if xroDisplayAlerts in Options then // BLD123
...
var
  ...
  function TestSheetName(const NameIndex: integer; const Name: string): string;
  var
    i: integer;
  begin
    Result := Name;
  ...
  begin
    ...
    while not MSDataSet.Eof do begin
      ...
      // Set sheet name
      s := TestSheetName(1, MSDataSet.FieldAsString(MSFieldIndex));
      if s <> '' then begin
        FIMultisheet.Name := s;
        AddedNames.Add(s);
      end
    ...
    while not MSDataSet.Eof do begin
      ...
      if MSFieldValues.IndexOf(MSDataSet.FieldAsString(MSFieldIndex)) = -1 then begin
        ...
        if s <> '' then begin
          ...
          FIMultisheet.Name := FIMultisheet.Name + '_' + IntToStr(FMultisheetIndex);
          AddedNames.Add(FIMultisheet.Name);
        end;
        // Add new sheet
        {$IFDEF XLR_BCB}
